...
public class SolrServiceImpl implements SearchService, IndexingService {
    ...
    public DiscoverResult search(Context context, DiscoverQuery discoveryQuery, boolean includeUnDiscoverable) throws SearchServiceException {
		DiscoverResult allResults = null;
		Iterator<String> it = discoveryQuery.getQuerys().iterator();
        ...
        try {
			...
			while(it.hasNext()){
				...
				discoveryQuery.setQuery(it.next());
				System.out.println("QUERY: " + discoveryQuery.getQuery());
				...
				result = retrieveResult(context, discoveryQuery, queryResponse);
			
				if(allResults == null){
					allResults = result;			
				}
				else{
					Iterator<DSpaceObject> itDspace = result.getDspaceObjects().iterator();
					while(itDspace.hasNext()){
						boolean idExists = false;
						DSpaceObject resultObj = itDspace.next();
						Iterator<DSpaceObject> itAllResults = allResults.getDspaceObjects().iterator();
						while(itAllResults.hasNext() && !idExists){
							if(resultObj.getID() == itAllResults.next().getID()){
								idExists = true;							
							}
						}

						if(!idExists){
							allResults.getDspaceObjects().add(resultObj);
						}
					}
					
					allResults.getFacetResults().putAll(result.getFacetResults());
				}
			}

			if(allResults == null){
				System.out.println("ALLRESULTS NULL");
				SolrQuery solrQuery = resolveToSolrQuery(context, discoveryQuery, includeUnDiscoverable);
				QueryResponse queryResponse = getSolr().query(solrQuery, SolrRequest.METHOD.POST);
				
				allResults = retrieveResult(context, discoveryQuery, queryResponse);
			}
			System.out.println("</SEARCH>");
            return allResults;
